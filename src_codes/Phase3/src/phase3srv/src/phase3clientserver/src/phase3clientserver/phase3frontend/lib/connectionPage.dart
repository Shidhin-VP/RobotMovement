...
class _ConnectionpageState extends State<Connectionpage> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      setState(() {
        ...
        channel.stream.listen((message) {
          var msg = jsonDecode(message)['values']['message'];
          var success = jsonDecode(message)['values']['success'];
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              content: Center(child: Text(msg.toString(),style: TextStyle(color: Colors.orangeAccent),)),
            ),
          );
        });
      });
  ...
  void sendRos(String command, {double x = 0.0, y = 0.0, z = 0.0, w = 0.0}) {
    try {
      final message = {
        "op": "call_service",
        "service": "Phase3ClientService",
        "args": {"command": command, "x": x, "y": y, "z": z, "w": w},
      };
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          ...
          if ((_moveArm.text.isNotEmpty ^ _moveto.text.isNotEmpty) &&
            ...
            sendRos(
              movetoTrimer.isNotEmpty ? "move to" : "move arm",
              x: movetoTrimer.isNotEmpty
                  ? double.parse(movetoTrimer[0])
                  : double.parse(movearmTrimer[0]),
              y: movetoTrimer.isNotEmpty
                  ? double.parse(movetoTrimer[1])
                  : double.parse(movearmTrimer[1]),
              z: movetoTrimer.isNotEmpty
                  ? double.parse(movetoTrimer[2])
                  : double.parse(movearmTrimer[2]),
              w: movetoTrimer.isNotEmpty ? 0.0 : double.parse(movearmTrimer[3]),
            );
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
